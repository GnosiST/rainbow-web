...
- [x] 1. 初始化 Next.js 项目
  - [x] 1.1 创建 Next.js App Router 项目（TypeScript + Tailwind + ESLint）
...
- [x] 3. 配置 Tailwind 和全局样式
  - [x] 3.1 配置 tailwind.config.ts
    - 设置 content 路径
    - 添加自定义主题扩展（为后续 Settings 功能预留）
    - _Requirements: 1.3_
  - [x] 3.2 配置 globals.css
    - 引入 Tailwind 指令
    - 添加 CSS 变量占位（--brightness, --filter）
    - _Requirements: 1.3_

- [x] 4. 实现健康检查接口
  - [x] 4.1 创建 /api/health 路由
    - 返回 { status, timestamp }
...
- [x] 5. Checkpoint - 本地开发环境验收
  - 确保 npm run dev 可运行
  - 确保 GET /api/health 返回 200
  - 如有问题请提出

- [x] 6. 配置 Docker 生产构建
  - [x] 6.1 配置 next.config.ts standalone 模式
...
- [x] 7. 配置 Docker Compose
  - [x] 7.1 创建 docker-compose.yml
    - 定义 app 服务
    - 定义 nginx 服务
    - 配置网络
...
- [x] 8. 配置 Nginx 反向代理
  - [x] 8.1 创建 nginx/conf.d/site.conf
    - upstream 配置
    - 反向代理规则
    - 静态资源缓存
    - gzip 压缩
